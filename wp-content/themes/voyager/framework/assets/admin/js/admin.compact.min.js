...
var param=$.param(data);param=param.replace(/\+/g,'%20');param=param.replace(/\'/g,'%27');$data.val(param);});};this.bind();};$.fn.thbSection=function(){return this.each(function(){$(this).data("thb-field-section",new THB_Section($(this)));});};})(jQuery);(function($){var THB_Slide=function(field){var self=this;this.bind=function(){field.find(".thb-btn-edit").on("click",function(e){var subtype=$(this).attr("data-subtype"),key=$(this).attr("data-key"),title=$(this).attr("title");self.edit(false,key,subtype,title);return false;});field.find(".thb-btn-clone").on("click",function(){self.clone_self();return false;});};this.clone_self=function(){var container=field.parents(".thb-fields-container.duplicable").first().data("container");var cloned_slide=field.clone();field.after(cloned_slide);thb_boot_fields(cloned_slide);container.reorderFields();return false;},this.edit=function(new_slide,key,subtype,title){var modal=new THB_Modal(key+'_edit_slide_'+subtype,title,function(data){$.each(data,function(name,value){field.find("[data-name='"+name+"']").attr("value",value);});if(subtype=="video"){var url=data.id;if(new_slide&&url===""){field.remove();return false;}
...
else if(window.getSelection){selection=window.getSelection();range=document.createRange();range.selectNodeContents(element);selection.removeAllRanges();selection.addRange(range);}};})(jQuery);;(function($){window.thb_sync_trip_stops=function(value){var stops_select=$("select[name='trip_stop']");stops_select.html("");$.each(voyager_trips,function(){if(this.id==value){$.each(this.stops,function(){stops_select.append("<option value='"+this.value+"'>"+this.label+"</option>");});}});stops_select.trigger("render");};window.thb_voyager_trip_add_stop=function(container,tpl,control){var stop_add_modal=new THB_Modal('voyager_trip_stop',control.text(),function(data,modal){var field=container.addField(tpl),input=$("input.thb-stop-value",field),edit_button=$(".thb-stop-edit",field);input.val($.param(data));var label=data["label"].trim()==""?edit_button.data("default-label"):data["label"].trim();edit_button.html(label);return false;});stop_add_modal.open({});};window.thb_voyager_trip_stop_field=function($el){var input=$("input.thb-stop-value",$el),edit_button=$(".thb-stop-edit",$el);edit_button.on("click",function(){var stop_edit_modal=new THB_Modal('voyager_trip_stop',edit_button.text(),function(data,modal){input.val($.param(data));var label=data["label"].trim()==""?edit_button.data("default-label"):data["label"].trim();edit_button.find("a").html(label);return false;});stop_edit_modal.open($.deparam(input.val()));return false;});};window.thb_voyager_latlong_field=function($el){var latlong_field=this,input=$el.find(".thb-latlong-value"),input_search=$el.find(".thb-latlong-search").get(0),map=$el.find(".thb-latlong-map").get(0),form=$el.parents("form");latlong_field.center_marker=null;var get_map_center=function(){if(input.val()!=''){var latlong=input.val().split(",");if(latlong[0]&&latlong[1]){return new google.maps.LatLng(latlong[0],latlong[1]);}}
return new google.maps.LatLng(51.48257659999999,-0.007658900000024005);};var latlong_map=new google.maps.Map(map,{"zoom":16,"mapTypeId":google.maps.MapTypeId.ROADMAP,"streetViewControl":false,"center":get_map_center()})
var reposition_marker=function(position){if(latlong_field.center_marker){latlong_field.center_marker.setMap(null);}
latlong_field.center_marker=new google.maps.Marker({position:position,map:latlong_map,draggable:true,});google.maps.event.addListener(latlong_field.center_marker,'dragend',function(event){var latlong=latlong_field.center_marker.getPosition();input.val(latlong.lat()+","+latlong.lng());});};$(window).on("resize",function(){google.maps.event.trigger(latlong_map,'resize');reposition_marker(get_map_center());latlong_map.setCenter(get_map_center());});input.on("change keyup",function(){reposition_marker(get_map_center());latlong_map.setCenter(get_map_center());});$(input_search).on("keydown",function(e){if(e.which==13){return false;}});latlong_map.controls[google.maps.ControlPosition.TOP_LEFT].push(input_search);var searchBox=new google.maps.places.SearchBox(input_search);google.maps.event.addListener(searchBox,'places_changed',function(){var places=searchBox.getPlaces();if(places.length==0){return false;}
reposition_marker(places[0].geometry.location);input.val(places[0].geometry.location.lat()+","+places[0].geometry.location.lng());latlong_map.setCenter(get_map_center());return false;});google.maps.event.addListener(latlong_map,"click",function(event){var lat=event.latLng.lat(),lng=event.latLng.lng(),latlong=new google.maps.LatLng(lat,lng);reposition_marker(latlong);input.val(lat+","+lng);});};$(window).on("thb_boot_fields",function(event,root){if(root.hasClass("thb-field")){if(root.hasClass("thb-field-voyager_trip_stop")){thb_voyager_trip_stop_field(root);}
else if(root.hasClass("thb-field-voyager_latlong")){thb_voyager_latlong_field(root);}}
else{$(".thb-field-voyager_trip_stop",root).each(function(){thb_voyager_trip_stop_field($(this));});$(".thb-field-voyager_latlong",root).each(function(){thb_voyager_latlong_field($(this));});}});})(jQuery);